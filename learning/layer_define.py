...
def conv(inputs, channel_size=1, layer_size=32, kernel_size=3, pooling=True, conv_strides=[1, 1, 1, 1], maxpool_ksize=[1, 2, 2, 1], maxpool_strides=[1, 2, 2, 1]):
    W_conv = tf.Variable(tf.compat.v1.truncated_normal([kernel_size, kernel_size, channel_size, layer_size], stddev=0.1))
    conv = tf.nn.conv2d(inputs, W_conv, strides=conv_strides, padding='SAME')
    b_conv = tf.Variable(tf.compat.v1.truncated_normal([layer_size], stddev=0.1))
...
def fc(inputs, input_size, layer_size, dropout_rate):
    W_fc = tf.Variable(tf.compat.v1.truncated_normal([input_size, layer_size], stddev=0.1))
    b_fc = tf.Variable(tf.compat.v1.truncated_normal([layer_size], stddev=0.1))
...
def out(input, input_size, class_size=2):
    W_output = tf.Variable(tf.compat.v1.truncated_normal([input_size, class_size], stddev=0.1))
    b_output = tf.Variable(tf.compat.v1.truncated_normal([class_size], stddev=0.1))
